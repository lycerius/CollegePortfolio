...
import edu.wit.cs.comp3370.LAB8;
...
public class ChartMaker extends ApplicationFrame {
    ...
    static class AddRuns implements Function2D {
    	...
    	public double getValue(double t) {
    		...
    		for (int i = 0; i < 20; i++) {
    			LAB8.Item[] items = randItems(Double.valueOf(t).intValue());

    			long start = System.nanoTime();

    			switch (algo) {
    			case 'd':
    				LAB8.FindDynamic(items, 20);
    				break;
    			case 'e':
    				LAB8.FindEnumerate(items, 20);
    				break;
    			case 'g':
    				LAB8.FindGreedy(items, 20);
    				break;
    			default:
		...
		private LAB8.Item[] randItems(int numItems) {
			LAB8.Item[] items = new LAB8.Item[numItems];
		    Random r = new Random();

		    for (int i = 0; i < numItems; i++)
		        items[i] = new LAB8.Item(r.nextInt(10), r.nextInt(20), i);
		    return items;
		}
