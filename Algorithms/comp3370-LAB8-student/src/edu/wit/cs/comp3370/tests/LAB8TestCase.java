...
import edu.wit.cs.comp3370.LAB8.Item;
...
public class LAB8TestCase{
	...
	private void _testDyn(LAB8.Item[] items, int weight, int expectedBest) {
		LAB8.Item[] actual = new LAB8.Item[0];
		int actualBest = 0;
		
		try {
			actual = LAB8.FindDynamic(items, weight);
			actualBest = LAB8.getBest();
	...
	private void _testFileDyn(String file, int weight, int expectedBest) {
		...
		try (Scanner f = new Scanner(new File(file))) {
			int i = 0;
			while(f.hasNextInt())
				tableList.add(new LAB8.Item(f.nextInt(), f.nextInt(), i++));
		} catch (IOException e) {
